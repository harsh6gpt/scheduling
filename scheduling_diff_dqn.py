...
for j in range(num_episodes):
    ...
    for i in range(time_steps):
        ...
        len_q1 = len_q + arrivals
        channel = np.random.binomial(1, dep_rate)
        len_q1, lost_count = q_proj(len_q1, buff_size, num_q)
        reward = np.sum(len_q1 - len_q)
        len_q = len_q1
        ...
        if mc_sim != 1:
            new_q[action] = reward + gamma*(np.amax(model.predict(np.reshape(state, [1, 2*num_q]))[0]))
            y_data.append(new_q)
        else:
            new_q[action] = reward
            y_data.append(new_q)
            for k in range(max(0, i - mc_time), i):
                y_data[k][action_data[k]] += math.pow(gamma, i - k)*reward
            if i - mc_time - 1 >= 0:
